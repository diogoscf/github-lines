...
describe("Test link matching and fetching (coreLogic.handleMessage)", () => {
  it("GitHub single line", async () => {
    const url = "https://github.com/diogoscf/github-lines/blob/master/LICENSE#L1";
    const { msgList, totalLines } = await coreLogic.handleMessage(url);
  ...
  it("Multi-snippet", async () => {
    ...
    const { msgList, totalLines } = await coreLogic.handleMessage(msg);
    expect(totalLines).to.equal(3);
